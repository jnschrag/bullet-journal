...
import { promiseArray, promiseObject } from "ember-data/-private/system/promise-proxies";

import { _bind, _guard, _objectIsAlive } from "ember-data/-private/system/store/common";
...
import { _find, _findMany, _findHasMany, _findBelongsTo, _findAll, _query as _query3, _queryRecord } from "ember-data/-private/system/store/finders";
...
Store = Service.extend({
  ...
  _flushPendingFetchForType: function _flushPendingFetchForType(pendingFetchItems, typeClass) {
    ...
    if (pendingFetchItems.length === 1) {
      ...
      groups.forEach(function (groupOfSnapshots) {
        ...
        if (ids.length > 1) {
          _findMany(adapter, store, typeClass, ids, requestedRecords).then(resolveFoundRecords).then(makeMissingRecordsRejector(requestedRecords)).then(null, makeRecordsRejector(requestedRecords));
  ...
  */
  ...
  _query: function _query(modelName, query, array) {
    ...
    return promiseArray(_query3(adapter, this, typeClass, query, array));
  },
...
function setupRelationships(store, record, data) {
  ...
  record.type.eachRelationship(function (key, descriptor) {
    ...
    if (data.relationships[key].links && data.relationships[key].links.related) {
      var relatedLink = _normalizeLink(data.relationships[key].links.related);
